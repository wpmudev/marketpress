...
class MP_Gateway_eWay31 extends MP_Gateway_API {
	...
	function process_payment( $cart, $billing_info, $shipping_info ) {
		...
		if ( ! $cart->is_download_only() && mp_get_setting( 'shipping->method' ) != 'none' && mp_get_post_value( 'enable_shipping_address' ) ) {	
			...
			$request->Customer->FirstName = mp_arr_get_value( 'first_name', $shipping_info );
			$request->Customer->LastName = mp_arr_get_value( 'last_name', $shipping_info );
			$request->Customer->Street1 = mp_arr_get_value( 'address1', $shipping_info );
			
			if ( $address2 = mp_arr_get_value( 'address2', $shipping_info ) ) {
				$request->Customer->Street2 = mp_arr_get_value('address2', $shipping_info);
			}
