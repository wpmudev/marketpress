...
class MP_Gateway_Bitpay extends MP_Gateway_API {
  ...
  var $currency = '';
	...
	function on_creation() {
		...
		$this->redirectMessage   = $this->get_setting('redirectMessage');
  ...
  /**
    ...
    if ( ! mp_get_setting('tax->tax_inclusive') ) {
		...
		if ( isset( $invoice->error ) ) {
			...
			$iframe = $this->get_setting('iframe');
	...
	function order_confirmation( $order ) {
		...
		$bitpayInvoiceId = mp_get_session_value('bitpayInvoiceId', '');
      ...
      //setup our payment details
			...
			$mp_shipping_info = mp_get_session_value('mp_shipping_info', '');
			$order = mp()->create_order( mp_get_session_value('mp_order'), mp()->get_cart_contents(), $mp_shipping_info, $payment_info, false );
		} else {
  ...
  function init_settings_metabox() {
	  $metabox = new WPMUDEV_Metabox(array(
		...
		$metabox->add_field('checkbox', array(
			...
			'label' => array('text' => __('Embed Iframe For Payment', 'mp')),
			'message' => __('Yes', 'mp'),
			'value' => 'yes',
			'desc' => __('If checked, iframe will be embeded at payment page, otherwise user will be redirected to BitPay site for payment.', 'mp'),
		));
		$metabox->add_field('textarea', array(
			'name' => $this->get_field_name('redirectMessage'),
			'label' => array('text' => __('Redirect Message', 'mp')),
			'desc' => __('Displayed on payment page.', 'mp'),
	...
	function gateway_settings_box( $settings ) {
		$transactionSpeed = mp_get_setting( 'gateways->bitpay->transactionSpeed', 'high' );
		$fullNotifications = mp_get_setting( 'gateways->bitpay->fullNotifications', 'no' );
		$redirectMessage = mp_get_setting( 'gateways->bitpay->redirectMessage', 'You will be redirected to <a href="http://bitpay.com" title="">bitpay.com</a>, for bitcoin payment. It is completely safe.' );
		$iframe = mp_get_setting( 'gateways->bitpay->iframe' );
		$debugging = mp_get_setting( 'gateways->bitpay->debugging' );
		$private_key = mp_get_setting( 'gateways->bitpay->private_key' );
	...
	function process_payment( $cart, $shipping_info ) {
		...
		;
		$iframe_enabled = mp_get_setting( 'gateways->bitpay->iframe' );
		$private_key = mp_get_setting( 'gateways->bitpay->private_key' );
	...
	function bitpay_request_url( $url, $apiKey, $post = false ) {
		...
		if ( mp_get_setting( 'gateways->bitpay->debugging' ) ) {
			$this->bitpay_log( $response );
		}
		return $response;
	}
