...
class MP_Gateway_PIN extends MP_Gateway_API {
    ...
    function on_creation() {
        ...
        $this->public_key = mp_get_setting('gateways->pin->public_key');
        $this->private_key = mp_get_setting('gateways->pin->private_key');
        $this->force_ssl = mp_get_setting('gateways->pin->is_ssl');
        $this->currency = mp_get_setting('gateways->pin->currency', 'AUD');
    ...
    function enqueue_scripts() {
        ...
        if (!is_admin() && get_query_var('pagename') == 'cart' && get_query_var('checkoutstep') == 'checkout') {
            ...
            if (mp_get_setting('gateways->pin->is_ssl')) {
                    ...
                    <tr valign="top">
                        ...
                        <td>
                            ...
                            <select name="mp[gateways][pin][is_ssl]">
                                <option value="1"<?php selected(mp_get_setting('gateways->pin->is_ssl'), 1); ?>><?php _e('Force SSL (Live Site)', 'mp') ?></option>
                                <option value="0"<?php selected(mp_get_setting('gateways->pin->is_ssl', 0), 0); ?>><?php _e('No SSL (Testing)', 'mp') ?></option>
                            </select>
                        </td>
                    </tr>
                    <tr>
                        ...
                        <td>
                            ...
                            <p><label><?php _e('Secret API Key', 'mp') ?><br />
                                    <input value="<?php echo esc_attr(mp_get_setting('gateways->pin->private_key')); ?>" size="70" name="mp[gateways][pin][private_key]" type="text" />
                                </label></p>
                            <p><label><?php _e('Publishable API Key', 'mp') ?><br />
                                    <input value="<?php echo esc_attr(mp_get_setting('gateways->pin->public_key')); ?>" size="70" name="mp[gateways][pin][public_key]" type="text" />
                                </label></p>
                        </td>
                    </tr>
                    <tr valign="top">
                        ...
                        <td>
                            ...
                            <select name="mp[gateways][pin][currency]">
                                <?php
                                $sel_currency = mp_get_setting('gateways->pin->currency', $settings['currency']);
                                $currencies = array(
                                    "AUD" => 'AUD',
                                    "NZD" => 'NZD',
                                    "USD" => 'USD',
