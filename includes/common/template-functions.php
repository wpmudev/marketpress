...
function mp_related_products() {
	...
	if( mp_get_setting('related_products->show') == 0)
		return '';
...
function _mp_cart_shipping($editable = false, $echo = false) {
		...
		if (!$country)
				$country = mp_get_setting('base_country', 'US');
		...
		if ($editable) {
			 ...
			 $content .= apply_filters('mp_checkout_before_shipping', '');
				...
				$content .= '<thead><tr>';
				$content .= '<th colspan="2">' . (((mp()->download_only_cart(mp()->get_cart_contents()) && !mp()->global_cart) || mp_get_setting('shipping->method') != 'none') ? __('Enter Your Checkout Information:', 'mp') : __('Enter Your Shipping Information:', 'mp')) . '</th>';
				$content .= '</tr></thead>';
				$content .= '<tbody>';
				$content .= '<tr>';
				...
				if ((!mp()->download_only_cart(mp()->get_cart_contents()) || mp()->global_cart || mp_get_setting('tax->downloadable_address')) && mp_get_setting('shipping->method') != 'none') {
						$content .= '<tr>';
				...
				if (mp_get_setting('special_instructions')) {
						$content .= '<tr>';
...
function _mp_cart_payment($type, $echo = false) {
		...
		$active_gateways = MP_Gateway_API::get_active_gateways();

		$content = '';
		if ($type == 'form') {
				$content = '<form id="mp_payment_form" method="post" action="' . mp_checkout_step_url('checkout') . '">';
				if (count((array) $active_gateways) == 1) {
						$content .= '<input type="hidden" name="mp_choose_gateway" value="' . $active_gateways[0]->plugin_name . '" />';
				} else if (count((array) $active_gateways) > 1) {
						$content .= '<table class="mp_cart_payment_methods">';
						$content .= '<thead><tr>';
						$content .= '<th>' . __('Choose a Payment Method:', 'mp') . '</th>';
						$content .= '</tr></thead>';
						$content .= '<tbody><tr><td>';
						foreach ((array) $active_gateways as $plugin) {
								$content .= '<label>';
...
function mp_show_cart($context = '', $checkoutstep = null, $echo = true) {
		...
		if (mp_items_in_cart() || $checkoutstep == 'confirmation') {
				...
				if ($context == 'widget') {
					...
					if (mp_get_setting('show_purchase_breadcrumbs') == 1) {
					...
					$content .= apply_filters('mp_checkout_error_checkout', '');
						...
						switch ($checkoutstep) {
								...
								case 'shipping':
										$content .= do_shortcode(mp_get_setting('msg->shipping'));
										$content .= _mp_cart_shipping(true);
										break;

								case 'checkout':
										$content .= do_shortcode(mp_get_setting('msg->checkout'));
										$content .= _mp_cart_payment('form');
										break;

								case 'confirm-checkout':
										$content .= do_shortcode(mp_get_setting('msg->confirm_checkout'));
								...
								case 'confirmation':
										$content .= do_shortcode(mp_get_setting('msg->success'));
										$content .= _mp_cart_payment('confirmation');
										break;

								default:
										$content .= do_shortcode(mp_get_setting('msg->cart'));
...
function mp_order_status( $echo = true ) {
		...
		$content = do_shortcode(mp_get_setting('msg->order_status'));
...
function mp_list_products() {
	...
	if ( (!is_null($args['paginate']) && !$args['paginate']) || (is_null($args['paginate']) && !mp_get_setting('paginate')) ) {
		...
		if ( is_null($args['order_by']) ) {
			if ( 'price' == mp_get_setting('order_by') ) {
...
function mp_product_image($echo = true, $context = 'list', $post_id = NULL, $size = NULL, $align = NULL) {
		...
		if ($context == 'list') {
				...
				if (intval($size)) {
						...
						if (mp_get_setting('list_img_size') == 'custom')
								$size = array(mp_get_setting('list_img_width'), mp_get_setting('list_img_height'));
						else
								$size = mp_get_setting('list_img_size');
				}
				...
				if (mp_get_setting('product_img_size') == 'custom')
						$size = array(mp_get_setting('product_img_width'), mp_get_setting('product_img_height'));
				else
...
function mp_products_filter( $hidden = false, $per_page = null ) {
		...
		$current_order = strtolower(mp_get_setting('order_by') . '-' . mp_get_setting('order'));
		$options = array(
...
function mp_store_link($echo = true, $url = false, $link_text = '') {
		;
		$link = home_url(trailingslashit(mp_get_setting('slugs->store')));
...
function mp_products_link($echo = true, $url = false, $link_text = '') {
		;
		$link = home_url(mp_get_setting('slugs->store') . '/' . mp_get_setting('slugs->products') . '/');
...
function mp_orderstatus_link($echo = true, $url = false, $link_text = '') {
		;
		$link = home_url(mp_get_setting('slugs->store') . '/' . mp_get_setting('slugs->orderstatus') . '/');
