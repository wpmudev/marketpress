	...
	public function tax_total( $format = false, $estimate = false, $include_shipping_tax = true ) {
		if ( false === mp_arr_get_value( 'tax', $this->_total ) ) {
			...
			while ( 1 ) {
				...
				foreach ( $items as $item ) {
					$table      = mp_tax()->find_product_table_rate( $item );
					$rates      = mp_tax()->find_rates( $table, false, $item->is_download() );
	...
	public function export_to_array() {
		...
		foreach ( $this->get_all_items() as $b_id => $items ) {
			...
			foreach ( $items as $id => $qty ) {
				...
				$table   = get_post_meta( $id, 'special_tax_rate', true );
				if ( ! $product->is_variation() ) {
					$data[ $id ][] = array(
						...
						'before_tax_price' => mp_tax()->get_product_original_price( $product->get_price( 'lowest' ), $table )
					);
				} else {
