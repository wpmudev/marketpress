...
function mp_list_global_products( $args = '' ) {
  ...
  if ($category) {
    ...
    $query = "SELECT * FROM {$wpdb->base_prefix}mp_products p INNER JOIN {$wpdb->base_prefix}mp_term_relationships r ON p.id = r.post_id INNER JOIN {$wpdb->base_prefix}mp_terms t ON r.term_id = t.term_id WHERE p.blog_public = 1 AND t.type = 'product_category' AND t.slug = '$category'";
  } else if ($tag) {
    $tag = $wpdb->escape( sanitize_title( $tag ) );
    $query = "SELECT * FROM {$wpdb->base_prefix}mp_products p INNER JOIN {$wpdb->base_prefix}mp_term_relationships r ON p.id = r.post_id INNER JOIN {$wpdb->base_prefix}mp_terms t ON r.term_id = t.term_id WHERE p.blog_public = 1 AND t.type = 'product_tag' AND t.slug = '$tag'";
  } else {
    $query = "SELECT * FROM {$wpdb->base_prefix}mp_products p WHERE p.blog_public = 1";
  }
  ...
  if ($results) {
    foreach ($results as $product) {
      ...
      //show content
      if ($text == 'excerpt') {
        $text = str_replace(']]>', ']]&gt;', $product->post_content);
